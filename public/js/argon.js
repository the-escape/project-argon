    ...
    dialog: {
        ...
        confirm: function (el, text) {
            if (!text) {
                ...
                text =
                    el.dataset.confirm || 'Are you sure you want to continue?'
...
function loadItems (id) {
    $.ajax(argon.root() + '/media/items', {
        ...
        for (var i in data) {
            ...
            node.find('[data-dz-size]').html(
                argon.helpers.filesize(file.filesize)
            )
...
$(document).on('click', '.field-file .field-add-file', function (e) {
    ...
    $('#medialib').on('hidden.bs.modal', function () {
        ...
        if (mlselect) {
            ...
            $.ajax(argon.root() + '/media/items/' + mlselect).done(function (
                data
            ) {
                var settings = JSON.parse(field.attr('data-settings'))
                var fieldName = field.attr('data-name')
                ...
                if (settings.multiple) {
                    $('<div/>')
                        .addClass('input-group-addon sortable-handle')
                        .text('⇅')
                        .appendTo(container)
                }

                $('<div/>')
                    .addClass('file-name form-control')
                    .text(data.filename + '.' + data.extension)
                    .appendTo(container)

                $('<div/>')
                    .addClass('input-group-addon field-remove')
                    .text('\u2715')
                    .appendTo(container)
...
function medialib (data) {
    if (window.opener) {
        $.ajax(argon.root() + '/media/items/' + data).done(function (r) {
            window.opener.CKEDITOR.tools.callFunction(
                argon.helpers.getUrlParam('CKEditorFuncNum'),
                r.url,
                ''
            )
            window.close()
...
$('#medialib').on('shown.bs.modal', function () {
    // correct here use 'shown.bs.modal' event which comes in bootstrap3
    $(this)
        .find('iframe')
        .attr('src', '/admin/media/modal/all')
})
    ...
    $('#medialib').on('hidden.bs.modal', function () {
        ...
        if (mlselect) {
            ...
            $.ajax(argon.root() + '/media/items/' + mlselect).done(function (
                ...
                $('<input type="hidden" />')
                    .attr('name', fieldName + '[id]')
                    .val(data.id)
                    .appendTo($content)
                $('<input type="hidden" />')
                    .attr('name', fieldName + '[width]')
                    .val(data.meta.width)
                    .appendTo($content)
                $('<input type="hidden" />')
                    .attr('name', fieldName + '[height]')
                    .val(data.meta.height)
                    .appendTo($content)
                ...
                $('<div/>')
                    .addClass('file-name')
                    .text('Name: ' + data.filename + '.' + data.extension)
                    .appendTo($content)
                $('<div/>')
                    .addClass('file-name')
                    .text('Size: ' + data.filesize_formatted)
                    .appendTo($content)
                $('<div/>')
                    .addClass('file-name')
                    .text(
...
var WYSIWYG = {
    ...
    init: function (el) {
        ...
        CKEDITOR.config.fontSize_sizes =
            '12px;13px;14px;16px;18px;20px;22px;24px;26px;27px;28px;30px;32px;'
    ...
    repopulate: function () {
        ...
        $('.ckeditor').each(function (i, el) {
            ...
            if (!$cke_editor || $cke_editor.length === 0) {
                ...
                CKEDITOR.config.extraAllowedContent = WYSIWYG.getExtraAllowedContent(
                    el
                )
                CKEDITOR.config.height = WYSIWYG.getHeight(el)
                CKEDITOR.config.format_tags = WYSIWYG.getFormatTagsOptions(el)
