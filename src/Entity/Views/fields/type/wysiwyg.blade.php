...
@endif
    ...
    @foreach($value as $k => $v)
        ...
        @endif
            ...
            <textarea name="{{ $field->getFormFieldName($hash) }}" class="form-control form__wysiwyg @if($field->isRequired()) required @endif" data-wysiwyg_height="{{$wysiwyg_config_height}}" data-wysiwyg_toolbar="{{$wysiwyg_config_toolbar}}" data-wysiwyg_format_tags="{{$wysiwyg_config_format_tags}}" data-wysiwyg_extraAllowedContent="{{$wysiwyg_config_extraAllowedContent}}">{{ $v }}</textarea>
