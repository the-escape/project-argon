<?php namespace Escape\Argon\Helper;
        ...
        if ($this->isEnabled()) {
            ...
            if (!$entities_to_index || in_array($entity->entity_type_id, $entities_to_index)) {
                ...
                foreach ($fields as $field) {
                    ...
                    if ($type instanceof \Escape\Argon\Entity\FieldTypes\ComboFieldType) {
                        ...
                        foreach ($values as $hash => $val) {
                            ...
                            foreach ($type->getSubFields() as $subField) {
                                ...
                                if ($subField instanceof \Escape\Argon\Entity\FieldTypes\ImageFieldType) {
                                    continue;

                                } elseif ($subField instanceof \Escape\Argon\Entity\FieldTypes\FileFieldType) {
                                    continue;

                                } elseif ($subField instanceof \Escape\Argon\Entity\FieldTypes\VideoFieldType) {
                                    continue;

                                } elseif ($subField instanceof \Escape\Argon\Entity\FieldTypes\BooleanFieldType) {
                                    // TODO: REVIEW HERE
                                ...
                                } elseif ($subField instanceof \Escape\Argon\Entity\FieldTypes\DatetimeFieldType) {
                                    $vals = $values->getValueForSubField($hash, $subField->getId());
                                    foreach ($vals as $val) {
                                        $v = (string) $val;
                                ...
                                } elseif ($subField instanceof \Escape\Argon\Entity\FieldTypes\ItemFieldType) {
                                    $vals = $values->getValueForSubField($hash, $subField->getId());
                                    $vals = $vals->getIds();
                    ...
                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\ImageFieldType) {
                        continue;

                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\FileFieldType) {
                        continue;

                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\VideoFieldType) {
                        continue;

                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\BooleanFieldType) {
                        $doc->addField($slug . "_is", (int)$values->isTrue());

                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\DatetimeFieldType) {
                    ...
                    } elseif ($type instanceof \Escape\Argon\Entity\FieldTypes\ItemFieldType) {
