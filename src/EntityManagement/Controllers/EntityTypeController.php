...
class EntityTypeController extends BaseController
{
    ...
    public function saveField(
        ...
        $fieldType = $fieldTypesManager->getType($this->request->get('field_type'));

        $settings = $fieldType->getDefaultSettings();

        $groupId = 0;

        if ($groupName = $this->request->get('group')) {
    ...
    public function updateField(
        ...
        if ($field->field_type == $fieldType->getKey())
        {
            ...
            foreach ($settings as $settingKey => &$settingValue)
            {
                foreach ($fieldSettings as $fieldSettingKey => &$fieldSettingValue)
                {
                    ...
                    $settingValue = $this->request->get($settingKey, $fieldSettingValue);
                    break;
                }
            }
        }
        ...
        if ($order = $this->request->get('options_order')) {
            if ($order = explode(',', $order)) {
                $fieldSettings = $field->settings;
                $settings->options = [];
    ...
    public function saveGroup($typeId, EntityGroupRepository $groupRepository)
    {
        ...
        $groupName = $this->request->get('name');
    ...
    public function updateCombo(
        ...
        if ($order = $this->request->get('subfields_order')) {
    ...
    public function saveOption(
        ...
        $settings->options[] = $this->request->get('name');
    ...
    public function updateOption(
        ...
        $option->value = $this->request->get('value', $optionId);
